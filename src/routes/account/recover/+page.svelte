<script lang="ts">
    ...
    import { sendForgotCode,confirmPasswordChange, CodeError, UserError } from './../../../serverContactor';
    ...
    function handleButton(event:any) {
        if(generatingNew) {
            ...
            confirmPasswordChange(code??"",password)
                .catch(err => {
                    if(err instanceof CodeError) modal.open($t("account_recovery_fail"),$t("account_recovery_fail_description"))
