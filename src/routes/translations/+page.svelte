...
<Holder>
	...
	{#if loaded}
		{#each $locales as loc, index}
			<div class="locale">
				<h3>
					<a href={`/translations/${loc}`}>{$t(`lang.${loc}`)}</a>
					<span style="text-shadow: rgba(0,0,0,0.5) 0px 0px 3px"
						>{getFlagEmoji(loc)}</span>
				</h3>
				<span style="display: flex; align-items: center;"
					><p>Coverage: {Math.round((translationPercentages.get(loc) ?? 1) * 100)}%</p>
					{#if index === 0}<Tooltip>How many sections are translated</Tooltip>{/if}</span>
			</div>
		{/each}
	{:else}
		{#each Array(4) as _}
