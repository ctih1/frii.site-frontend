<script lang="ts">
    ...
    function handleClick() {
        values.forEach((element,index)=>{
            console.log(`Values: ${indexes}. Index: ${index}. Value: ${element}`);
            if(!modified.includes({key:indexes.at(index) as string, val:element})) {
                modified.push({key:indexes.at(index) as string,val:element});
            }
        }); 
...
<Holder>
        ...
        {#if loaded}
            {#each keys as key, index}
                <div class="key">
                    ...
                    <input bind:value={values[index]} type="text" style="height: 2em;" placeholder={$t("common.translation_input")}>
                </div>
            {/each}
        {:else}
            {#each Array(16) as _}
    ...
    <Button on:click={()=>handleClick()} args="fill padding argin-1em-top">{$t("common.security_report_submit")}</Button>
</Holder>
