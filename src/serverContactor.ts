...
export const getServerURL = () =>
  localStorage.getItem("serverURL") || "https://api.frii.site";
...
export async function reportVulnerability(
  endpoint: string,
  expected: string,
  actual: string,
  importance: number,
...
export class ServerContactor {
  ...
  async domainAvailable(domain: string): Promise<Response> {
    return await fetch(
      `${this.serverURL}/domain-is-available?domain=${domain}`,
    );
  }
  async modifyDomain(
    domain: string,
  ...
  async submitLanguageContribution(
    language: string,
    contribution: { key: string; val: string }[],
  ) {
    return await fetch(
      `${this.serverURL}/translations/${language}/contribute`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
